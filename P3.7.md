...
0. Launch the setup Ubuntu-xx.xx distro using PowerShell. Subsequently, log in as the user hduser and start Hadoop-related Services using the hduser account. 
   ...
   > hduser@MyPC:~$ hdfs dfs -mkdir /tmp/hduser
   > hduser@MyPC:~$ hdfs dfs -mkdir /user
   > hduser@MyPC:~$ hdfs dfs -mkdir /user/hduser
   > hduser@MyPC:~$ hdfs dfs -mkdir /user/hduser/warehouse
   > hduser@MyPC:~$ hdfs dfs -mkdir /user/hduser/lib
   > hduser@MyPC:~$ hdfs dfs -chmod g+w /tmp
   > hduser@MyPC:~$ hdfs dfs -chmod g+w /tmp/hduser
   > hduser@MyPC:~$ hdfs dfs -chmod 777 /user
   > hduser@MyPC:~$ hdfs dfs -chmod g+w /user/hduser
   > hduser@MyPC:~$ hdfs dfs -chmod g+w /user/hduser/warehouse
   > hduser@MyPC:~$ hdfs dfs -chmod 777 /user/hduser/lib
   > hduser@MyPC:~$ mkdir /tmp/hive
   > hduser@MyPC:~$ chmod 777 /tmp/hive
   > ~~~
...
1. Configure and Start Hive-Related Services
   ...
   1.2 $\color{red}{\textsf{IMPORTANT}}$: Edit the hadoop-env.sh file:
    ~~~bash
    hduser@MyPC:~$ nano ~/hadoop3/etc/hadoop/hadoop-env.sh
    ~~~
    > **$\color{red}{\textsf{Uncomment}}$ the last line in the file.**
    > 
   ...
   1.3 Run HiveServer2
    ...
    ~~~
    > Note:	Leave the session running and $\color{red}{\textsf{DO NOT CLOSE}}$ the terminal after carrying out the above command.



2. **Log in as the user hduser in another session** to start Beeline CLI and Connect to the HiveServer. Beeline which is a Hive client to carry out CLI commands:
   ~~~bash
   hduser@MyPC:~$ jps
   hduser@MyPC:~$ cd ~/hive
   ...
   0: jdbc:hive2://>
   ~~~
   > Using the jps command, twelve (12) services are visible (order is not important): Jps, DataNode, NameNode, SecondaryNameNode, NodeManager, ResourceManager, QuorumPeerMain, Kafka, HMaster, HRegionServer, NetworkServerControl and **RunJar**
   > 
...
3. **Exercise**: Run Hive CLI Commands
   ...
   3.3 Create a new database
    ...
    3.5 Create Hive tables
      ...
      iv. Duplicate your data file, then overwrite the data in the Sales table with the contents of a text or CSV file. We assume sales.csv resides in your HDFS' target directory. Then, retrieve loaded records from the Sales table.
      ~~~bash
      ...
      v. **Your turn now**: Create another data file in HDFS, and append a set set of data in the Sales table. Then, retrieve updated records from the Sales table. Here we assume that the sales_new2.csv file is new created with four records in your HDFS’ directory named data. 
      ~~~bash
      0: jdbc:hive2://> LOAD DATA INPATH '/user/student/data/sales_new2.csv' INTO TABLE Sales;
      0: jdbc:hive2://> SELECT * from Sales;
      OK
      +-----------+--------------------+-------------------+-----------------+                         
      | sales.id  | sales.description  | sales.unit_price  | sales.quantity  |
...
4. **Advanced Exercise**: Assessing Hive using PySpark

   4.1 **Switch to user hduser**, ensure the /tmp/hive directory exists with the necessary permissions prior to changing to the student user.
      ~~~bash
      hduser@MyPC:~$ mkdir /tmp/hive
      hduser@MyPC:~$ chmod 777 /tmp/hive
   ...
   4.2 **Switch to user student**, ensure the student user's .bashrc file contains the following lines at its end.
      ~~~bash
      ...
      ~~~
      > **Hints**: Use **nano** to edit the file, followed by the **source** command, if needed

   4.3 Start the JupyterLab service using user **student**, and then access it in your browser
      ~~~bash
      hduser@MyPC:~$ cd ~
      hduser@MyPC:~$ su - student
      student@MyPC:~$ cd ~
   ...
   4.4 Review and execute the code in $\color{red}{\textsf{NOTEBOOK 3.7 PySpark and Hive.ipynb}}$ within the virtual environment, i.e. de-venv 

   4.5 Deactivate the virtual environment once you no longer need to work in the environment.
      ~~~bash
...
6. After done all your exercises, **stop the Hive-Related Services** with command as follows.
   ...
   5.1 Stop HiveServer2:
     ...
     > **$\color{red}{\textsf{ATTENTION}}$ and $\color{red}{\textsf{MUST DO THIS STEP}}$**: Remember to revert back the comment, as per the step 1.2 above (refer to file ~/hadoop3/etc/hadoop/hadoop-env.sh )
   ...
   5.2 Stop the Derby network server 
     ...
     ~~~
     > Note that your process ID may differ from the one shown above
   ...
   5.4 Stop Kafka
    ...
    5.6 Stop YARN and HDFS.
     ...
     hduser@MyPC:~/hadoop3$ jps
     ~~~
     > Confirm no extraneous services run beyond those reported by jps
