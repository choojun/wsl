...
3. Run Hive CLI Commands
   ...
   3.3 Create a new database
    ...
    3.5 Create Hive tables
      ...
      v. Exercise: Create another data file in HDFS, and append a set set of data in the Sales table. Then, retrieve updated records from the Sales table. Here we assume that the sales_new2.csv file is new created in your HDFS’ directory named data. 
      ~~~bash
...
4. **Advanced**: Assessing Hive using PySpark
   ...
   4.1 Ensure the /tmp/hive directory exists with the necessary permissions prior to changing to the student user
      ~~~bash
      hduser@MyPC:~$ mkdir /tmp/hive
      hduser@MyPC:~$ chmod 777 /tmp/hive
   ...
   4.2 Ensure the student user's .bashrc file contains the following lines at its end
      ~~~bash
      ...
      ~~~
      > Use **nano** to edit the file, followed by the **source** command, if needed
