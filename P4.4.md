...
4. Using HappyBase to work with HBase (using user student account)
   ...
   1. For the following past years questions, write code to store the data to HBase based on your data model design:
      * 202501: Question 2 b) (ii)
...
| Item | Descriptions|
| -- | -- |
| Problem | TTransportException | 
| Message | thriftpy2.transport.base.TTransportException: TTransportException(type=4, message='TSocket read 0 bytes') |
| Solution | Run the statement to create a connection to the localhost again and then the statement to create the table once again. |


| Item | Descriptions|
| -- | -- |
| Problem | Broken pipe error |
| Message | socket.error: [Errno 32] Broken pipe error |
| Solution | Check to ensure you have obtained the Table instance. <br /> If the error still exists, then kill the Thrift process and re-start it. |
