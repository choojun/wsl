...
## g. HBase Shell Commands

| Command | Description |
| ---- | ---- |
| create 'table_name', 'column_family1', 'column_family2' | Creates a table with specified column families. |
| disable 'table_name' | Disables a table before it can be altered. |
| enable 'table_name' | Re-enables a previously disabled table. |
| alter 'table_name', 'column_family' | Adds a new column family to an existing table. |
| describe 'table_name' | Displays the schema of a table. |
| list | Lists all the tables in the HBase database. |
| put 'table_name', 'row_key', 'column_family:column', 'value' | Inserts a value into a specific row and column in a table. |
| put 'table_name', 'row_key', 'column_family:column', 'new_value' | Updates a value in a specific row and column (same as the insert operation). |
| get 'table_name', 'row_key' | Retrieves a specific row from a table. |
| scan 'table_name' | Retrieves multiple rows from a table. |
