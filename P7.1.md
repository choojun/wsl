...
5. Crt-C to stop the tail command in step 3
...
hduser@PC25: $ hdfs dfs -getfacl -R /user
# file: /user
# owner: hduser
# group: supergroup
user::rwx
...
To verify it works, you can check the “lib” directory against the reported listing by getfacl
The highlighted color with the corresponding color is the same.

hduser@PC25: $ hdfs dfs -ls
Found 3 items
...
hduser@PC25:~$ mkdir myFolder
hduser@PC25:~$  nano ./myFolder/myFile.txt
hduser@PC25:~$ cat ./myFolder/myFile.txt
It is a wonderful day.
hduser@PC25:~$ mkdir ./myFolder/subFolder
hduser@PC25:~$ ls -l ./myFolder/
total 8
-rw-r--r-- 1 hduser hduser   23 Sep  3 00:55 myFile.txt
drwxr-xr-x 2 hduser hduser 4096 Sep  3 00:56 subFolder
hduser@PC25:~$ tar -cvf myDirectory.tar ./myFolder/
./myFolder/
./myFolder/myFile.txt
./myFolder/subFolder/
hduser@PC25:~$ gpg --symmetric --cipher-algo AES256 ./myDirectory.tar
...
Copy the gpg file to a local Windows folder.

hduser@PC25:/mnt/c$ sudo cp ~/myDirectory.tar.gpg /mnt/c/testFolder/
hduser@PC25:/mnt/c$ sudo shutdown now
hduser@PC25:/mnt/c$
PS C:\Users\kongh>

Terminate the current WSL distribution
Start another Linux in WSL

If wrong passphrase is given:
hduser@PC25:~$ gpg --decrypt myDirectory.tar.gpg > directory.tar
...
If correct passphrase entered, decryption is successful
...
hduser@PC25:~$ tar -xvf directory.tar
./myFolder/
./myFolder/myFile.txt
./myFolder/subFolder/
hduser@PC25:~$ ls -l ./myFolder/
