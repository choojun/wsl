# PRACTICAL 3.6: Spark Machine Learning (SparkKL)
...
**spark.ml** is Spark’s machine learning (ML) library inspired by scikit-learn. It provides a uniform set of high-level API built on top of DataFrames for constructing and tuning machine learning pipelines. Some related terminology:
  ...
  * **Transformer**: an algorithm which can transform one DataFrame into another DataFrame. Example, an ML model is a Transformer which transforms a DataFrame with features into a DataFrame with predictions.
  * **Estimator**: an algorithm which can be fit on a DataFrame to produce a Transformer. Example, a learning algorithm is an Estimator which trains on a DataFrame and produces a model.
  * **Pipeline**: A Pipeline chains multiple Transformers and Estimators together to specify an ML workflow. 

Refer to $\color{red}{\textsf{NOTEBOOK 3.6(a) SparkML Estimator and Transformer.ipynb}}$ for an example on the use of SparkML’s estimator and transformer.
...
Refer to $\color{red}{\textsf{NOTEBOOK 3.6 SparkML Pipeline.ipynb}}$ for an example on the use of SparkML’s pipeline.
